...
static void eth_initialize(struct net_if *iface)
{
	...
	smsc_read_mac_address(mac);

	SMSC9220->INT_EN |= BIT(enum_smsc9220_interrupt_rxstatus_fifo_level);
...
#else
	/* Currently broken, as size of net_buf's is no longer word-aligned,
	 * e.g. first bug contains 14-bytes eth header.
	 */
	frag = pkt->frags;
	while (frag) {
		res = smsc_write_tx_fifo(frag->data, frag->len,
					 frag->frags == NULL);
		if (res < 0) {
			goto error;
		}
...
static void eth_smsc911x_isr(struct device *dev)
{
	...
	if (int_status & BIT(enum_smsc9220_interrupt_rxstatus_fifo_level)) {
		...
		int_status &= ~BIT(enum_smsc9220_interrupt_rxstatus_fifo_level);
